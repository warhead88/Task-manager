...
В этом разделе показано, как происходит общение пользователя с ботом в формате "Запрос-Ответ".

### Добавление задачи (`/add`)
> **Вы:** `/add`
> **Бот:** 🖊 Напиши текст своей задачи. (Или отправь 'отмена', если передумал).
> **Вы:** Подготовить отчет по проекту
> **Бот:** ✅ Отлично! Я сохранил твою задачу.

### Просмотр задач (`/list`)
> **Вы:** `/list`
> **Бот:** 📋 **Твой список задач:**
> 1. Подготовить отчет по проекту

### Установка часового пояса (`/timezone`)
> **Вы:** `/timezone +3`
> **Бот:** ✅ Часовой пояс успешно установлен: **UTC +3**
*(Позволяет боту правильно рассчитывать локальное время при установке напоминаний)*

### Установка напоминания (`/remind`)
Обязательно наличие хотя бы одной задачи в `/list`.
> **Вы:** `/remind`
> **Бот:** ⏰ **Настройка напоминания**
> Твои задачи:
> 1. Подготовить отчет по проекту
> Напиши номер задачи, для которой хочешь включить напоминание
> **Вы:** `1`
> **Бот:** 🔄 Как часто напоминать? *(Инлайн-кнопки: Единожды / Каждый день / По будням ...)*
> **Вы:** *(Нажимаете "Единожды")*
> **Бот:** Выбрано: **Единожды**.
> Теперь напиши время в формате ЧЧ:ММ (например, 14:30).
> У тебя установлен часовой пояс: **UTC +3**.
> **Вы:** `18:00`
> **Бот:** ✅ Готово! Напомню о задаче 'Подготовить отчет по проекту' в 18:00 (по твоему времени).

### Выполнение задачи (`/done`)
> **Вы:** `/done 1`
> **Бот:** 🎉 Поздравляю! Ты выполнил задачу: **Подготовить отчет по проекту**
*(Напоминание для этой задачи будет автоматически отключено, а счетчик выполненных задач в `/stats` увеличится)*
...
Тесты проверяют логику смещения часовых поясов, расчет времени следующих напоминаний (recurrence) и интеграцию с базой данных на основе In-Memory SQLite.

Для локального запуска тестов необходимо установить зависимости из `pyproject.toml` в виртуальное окружение, а затем запустить команду:
